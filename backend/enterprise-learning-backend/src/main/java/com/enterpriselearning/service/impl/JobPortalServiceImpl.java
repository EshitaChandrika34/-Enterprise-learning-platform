...
public class JobPortalServiceImpl implements JobPortalService {
    ...
    @Override
    @Transactional
    public JobPostingResponse createJobPosting(JobPostingRequest request, String postedByEmail) {
        User postedBy = null;
        if (postedByEmail != null) {
            postedBy = userRepository.findByEmail(postedByEmail).orElse(null);
        }
        ...
        JobPosting jobPosting = JobPosting.builder()
                ...
                .status(request.getStatus() != null ? request.getStatus() : JobPostingStatus.OPEN)
                .postedBy(postedBy)
                .build();
    ...
    public List<JobPostingResponse> getAllJobPostings(JobPostingStatus status, String department, String search) {
        List<JobPosting> list = jobPostingRepository.searchJobPostings(status, department, search);
        return list.stream()
    ...
    @Override
    @Transactional
    public JobApplicationResponse applyForJob(JobApplicationRequest request, String applicantEmail) {
        User applicant = userRepository.findByEmail(applicantEmail)
                .orElseThrow(() -> new ResourceNotFoundException("User not found with email: " + applicantEmail));

        JobPosting job = jobPostingRepository.findById(request.getJobPostingId())
                .orElseThrow(() -> new ResourceNotFoundException("Job posting not found with id: " + request.getJobPostingId()));

        if (job.getStatus() == JobPostingStatus.CLOSED) {
            throw new BadRequestException("This job posting is closed and is no longer accepting applications.");
        }

        if (jobApplicationRepository.existsByApplicantIdAndJobPostingId(applicant.getId(), job.getId())) {
            throw new BadRequestException("You have already applied for this position: " + job.getTitle());
        }

        double matchScore = calculateBasicMatchScore(applicant, job);
    ...
    @Override
    @Transactional(readOnly = true)
    public List<JobApplicationResponse> getMyApplications(String applicantEmail) {
        User applicant = userRepository.findByEmail(applicantEmail)
                .orElseThrow(() -> new ResourceNotFoundException("User not found with email: " + applicantEmail));
    ...
    @Override
    @Transactional(readOnly = true)
    public List<JobApplicationResponse> getApplicationsByEmployee(Long employeeId) {
        if (!userRepository.existsById(employeeId)) {
            throw new ResourceNotFoundException("Employee not found with id: " + employeeId);
        }
        return jobApplicationRepository.findByApplicantId(employeeId).stream()
                .map(this::mapToApplicationResponse)
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional
    public JobApplicationResponse updateApplicationStatus(Long applicationId, ApplicationStatusUpdateRequest request) {
    ...
    public void withdrawApplication(Long applicationId, String applicantEmail) {
        ...
        if (!app.getApplicant().getId().equals(user.getId()) && user.getRole() != Role.ADMIN) {
    ...
    private double calculateBasicMatchScore(User applicant, JobPosting job) {
        List<String> reqSkills = parseCommaList(job.getRequiredSkills());
        if (reqSkills.isEmpty()) {
            return 100.0;
        }

        List<EmployeeSkill> empSkills = employeeSkillRepository.findByUserId(applicant.getId());
        Set<String> empSkillNames = empSkills.stream()
                .map(es -> es.getSkill().getName().trim().toLowerCase())
                .collect(Collectors.toSet());

        long matchCount = reqSkills.stream()
                .filter(s -> empSkillNames.contains(s.toLowerCase()))
                .count();

        double skillRatio = (double) matchCount / reqSkills.size();
        return Math.round(skillRatio * 100.0 * 10.0) / 10.0;
    }
    ...
    private JobPostingResponse mapToJobPostingResponse(JobPosting job) {
        long appCount = jobApplicationRepository.countByJobPostingId(job.getId());
        return JobPostingResponse.builder()
                ...
                .requiredSkillList(parseCommaList(job.getRequiredSkills()))
                .minExperienceYears(job.getMinExperienceYears())
                .status(job.getStatus())
                .postedById(job.getPostedBy() != null ? job.getPostedBy().getId() : null)
                .postedByName(job.getPostedBy() != null ? job.getPostedBy().getFirstName() + " " + job.getPostedBy().getLastName() : "Human Resources")
    ...
    private JobApplicationResponse mapToApplicationResponse(JobApplication app) {
        return JobApplicationResponse.builder()
                ...
                .jobDepartment(app.getJobPosting().getDepartment())
                ...
                .applicantEmployeeId(app.getApplicant().getEmployeeId())
